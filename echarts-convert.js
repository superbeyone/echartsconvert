...
;(function (window, document, undefined) {
    ...
    Convert.prototype.serverCheckAndSet = function (params, args) {
        ...
        if(args.registerMap){
			params.registerMap = JSON.parse(args.registerMap);
        }
        return true;
    };
    ...
    function createEchartsDom(params) {
        ...
        // body背景设置为白色
        $(document.body).css('backgroundColor', 'rgba(0,0,0,0)');
        // echarts容器
        var container = $("<div>")
            .attr('id', 'container')
            .css({
		...
		if(params.registerMap){
            try {
                echarts.registerMap(params.registerMap.key, JSON.parse(params.registerMap.value));
            } catch (error) {
                console.log(error)
            }
